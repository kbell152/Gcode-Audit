...
# Version: v3
# Date:    05/02/26
# File:    gcode_audit_v3_050226.py
# ============================================
#
# This pass: parser expansion + normalization + tokenizer rewrite.
# No new validators added in this pass (per agreed scope).
#
# Changes from v2:
#   - State now tracks full modal context (motion, distance, units,
#     plane, spindle, feed, tool, speed) in addition to X/Y/Z.
#   - Codes are normalized at parse time (G01 -> G1, M03 -> M3).
#   - Proper tokenizer handles space-separated AND no-space formats
#     (e.g. "G1X10Y20" splits correctly).
#   - Inline comments "(...)" and ";..." are stripped before tokenizing.
#   - Line classification: EMPTY, COMMENT, CODE.
#   - validate_spindle_and_feed reads modal state instead of re-scanning
#     tokens (this fixes the M03/M3 bug as a side effect).
#
# Backward-compatible parser output shape:
#   { "lines": [ { "line_index", "state", "tokens", "line_type" }, ... ] }
# Existing validators that only read state["X"], state["Y"], state["Z"]
...
def parse_gcode_lines(raw_lines):
    ...
    for i, raw_line in enumerate(raw_lines, start=1):
        ...
        # Apply tokens to state. We process in two passes because
        # parameter words (X, Y, Z, F, S) on the same line as a motion
        # G-code apply to that motion regardless of token order.
...
def _apply_tokens_to_state(tokens, state, line_index):
    """
    Update the modal state dict in place based on the tokens on one line.

    Modal rules:
      - G/M codes update their respective modal group.
      - X/Y/Z update position (absolute interpretation; G91 incremental
        handling is NOT yet implemented — flagged in TODO below).
      - F/S/T update their parameter modals.
      - Unknown tokens are silently ignored at the parser level (they
        are preserved in the tokens list for downstream inspection).

    TODO (future pass): when state["distance"] == "G91", X/Y/Z values
    should be added to the previous position instead of replacing it.
    Currently this parser assumes G90 (absolute) interpretation. This
    is a known limitation and validators should not rely on position
    accuracy when distance mode is G91 until this is implemented.
    """
    for token in tokens:
        if not token:
            continue
        ...
        if letter == "G":
            ...
            try:
                ...
                # Malformed coordinate; preserve trace, don't crash
                pass
        elif letter == "Y":
            try:
                state["Y"] = float(rest)
            ...
            try:
                # Tool numbers are usually integer
